...
int convertH5ToGeoTIFF(const char *subDataset, const char *outputFilename,
                       double *geoTransform, const char *projection, int width,
                       int height) {
  // Open the subdataset
  ...
  GDALDatasetH hOutDataset =
      GDALCreate(hDriver, outputFilename, width, height, 1, GDT_UInt16, NULL);
  if (hOutDataset == NULL) {
  ...
  CPLErr err = GDALRasterIO(hBand, GF_Read, 0, 0, width, height, pData, width,
                            height, GDT_UInt16, 0, 0);
  if (err != CE_None) {
  ...
  err = GDALRasterIO(hOutBand, GF_Write, 0, 0, width, height, pData, width,
                     height, GDT_UInt16, 0, 0);
  if (err != CE_None) {
...
//     const char *subDataset =
//     "HDF5:\"/home/jayavishnu/Documents/1738SIH/geoTIFF/COG/pipeline/SIH2024/3RIMG_04SEP2024_1015_L1B_STD_V01R00.h5\"://IMG_MIR";
//     const char *outputFilename = "output.tif";
//
//     // Geotransform parameters (based on metadata)
//     double geoTransform[6] = { 634.8015441962551222, 4.546763628841097749, 0,
//     6034.3929117445522934, 0, -4.546763628841097749 };
//
...
//     return convertH5ToGeoTIFF(subDataset, outputFilename, geoTransform,
