...
int process_dataset(hid_t file_id, const char *dataset_name,
  ...
  if (GDALRasterIO(hBand, GF_Write, 0, 0, dims[2], dims[1], data, dims[2],
                   dims[1], GDT_Float32, 0, 0) != CE_None) {
...
int process_hdf5_to_cog(const char *file_name, const char *output_file,
  ...
  double geotransform[6] = {
      ...
      (attrs.right_longitude - attrs.left_longitude) /
          1737,             // Pixel width (longitude resolution)
      0,                    // Rotation (X axis)
      attrs.upper_latitude, // Top-left Y coordinate
      0,                    // Rotation (Y axis)
      -(attrs.upper_latitude - attrs.lower_latitude) /
          1616 // Pixel height (latitude resolution)
  };
#pragma openmp for
  for (int i = 0; i < 6; i++) {
  ...
  GDALDatasetH hOutput =
      GDALCreate(hDriver, output_file, 1737, 1616, dataset_count, GDT_Float32,
                 creation_options);
  if (!hOutput) {
...
int PipelineMainL1C(char *filename, char *output_file) {
  ...
  if (process_hdf5_to_cog(filename, output_file, DATASET_NAMES,
                          DATASET_COUNT) != 0) {
...
// gcc -o get_sub get_sub.c geo_attributes.c -lhdf5 -I/usr/include/hdf5/serial
// -L/usr/lib/x86_64-linux-gnu/hdf5/serial -lgdal
