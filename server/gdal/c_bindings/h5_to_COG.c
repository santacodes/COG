...
int process_dataset(hid_t file_id, const char *dataset_name, const char *output_file, int width, int height, double geotransform[6]) {
    ...
    GDALDatasetH hOutput = GDALCreate(hDriver, output_file, width, height, 1, GDT_Float32, creation_options);
    if (!hOutput) {
    ...
    printf("Dataset: %s, Min: %.2f, Max: %.2f\n", dataset_name, minmax[0], minmax[1]);
...
int process_hdf5_to_cog(const char *file_name, const char *output_dir,
    ...
    for (int i = 0; i < dataset_count; i++) {
        ...
        snprintf(output_file, sizeof(output_file), "%s/%s.tif", output_dir, dataset_names[i]);

        printf("Processing dataset: %s -> %s\n", dataset_names[i], output_file);

        if (process_dataset(file_id, dataset_names[i], output_file, width, height, geotransform) != 0) {
...
int main() {
    ...
    if (process_hdf5_to_cog(file_name, output_dir, DATASET_NAMES, DATASET_COUNT) != 0) {
